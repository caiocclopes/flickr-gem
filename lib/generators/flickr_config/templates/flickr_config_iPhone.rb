    ...
    def setPriority
       ...
       cursor = Flickr.getAll[posicao]
        ...
        while cursor != nil # enquanto houver registros
          ...
          if !cursor.people.eql?("-") 
            cursor.priority = 1
            cursor.save
          elsif cursor.photos.eql?("flickr.photos.search")
            cursor.priority = 3
            cursor.save
          else 
            cursor.priority = 2
     ...
     while cursor != nil # enquanto houver registros
       ...
       if cursor.priority == 1
          if cursor.people.eql?("email")
              cursor.NSID = flickr.people.findByEmail(:find_email => cursor.email_or_username).nsid
              cursor.save
          else 
